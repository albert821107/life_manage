...
|------|------|
| `accounting` | 收支記錄 |
| `tasks` | `fitness` | `investments` | 持倉（台股、美股、加密貨幣、債券） |
...
```
> `wslInheritsWindowsSettings: true` 讓 Claude 的 Bash 工具跑在 WSL Linux 環境，確保與 Mac 行為一致（grep / find / sed 等指令全部可用）。
...
│   ├── 1-2  資產       data-sec="investment"  → #section-investment
│   │         └── 子頁籤 (switchInvTab)
│   │             ├── 1-2-1  資產總覽   tab='assets'  → #inv-sub-assets
│   │             ├── 1-2-2  台股       tab='tw'      → #inv-sub-market
│   │             ├── 1-2-3  美股       tab='us'      → #inv-sub-market
│   │             ├── 1-2-4  加密貨幣   tab='crypto'  → #inv-sub-market
│   │             └── 1-2-5  存款       tab='forex'   → #inv-sub-forex
│   ├── 1-3  AI助理     data-sec="ai"          → #section-ai
│   ├── 1-4  通知       data-sec="line"        → #section-line
│   ├── 1-5  旅遊       data-sec="travel"      → #section-travel
│   │         └── 子頁籤 (switchTravelTab)
│   │             ├── 1-5-1  總覽       tab='weekly'    → #travel-sub-weekly
