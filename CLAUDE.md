...
左側導航欄 (sidebar)
├── [基本功能]  #nav-basic
│   ├── 概覽       data-sec="dashboard"   → #section-dashboard
│   ├── 記帳       data-sec="accounting"  → #section-accounting
│   ├── 任務       data-sec="tasks"       → #section-tasks
│   ├── 健身       data-sec="fitness"     → #section-fitness
│   ├── 資產       data-sec="investment"  → #section-investment
│   │   └── 子頁籤 (inv-tab / switchInvTab)
│   │       ├── 資產總覽   tab='assets'      → #inv-sub-assets
│   │       ├── 台股       tab='tw'          → #inv-sub-market
│   │       ├── 美股       tab='us'          → #inv-sub-market
│   │       ├── 加密貨幣   tab='crypto'      → #inv-sub-market
│   │       └── 存款       tab='forex'       → #inv-sub-forex
│   ├── AI助理     data-sec="ai"          → #section-ai
│   ├── 通知       data-sec="line"        → #section-line
│   ├── 旅遊       data-sec="travel"      → #section-travel
│   │   └── 子頁籤 (inv-tab / switchTravelTab)
│   │       ├── 總覽       tab='weekly'      → #travel-sub-weekly
│   │       ├── 行程       tab='trips'       → #travel-sub-trips
│   │       ├── 回憶       tab='memories'    → #travel-sub-memories
│   │       └── 足跡       tab='footprint'   → #travel-sub-footprint
│   └── 遊戲       data-sec="games"       → #section-games
│
└── [進階功能]  #nav-advanced
    ├── 工作       data-sec="work"        → #section-work
    │   └── 子頁籤 (inv-tab / switchWorkTab)
    │       └── Jira工單   tab='jira'        → #work-sub-jira
    ├── 投資       data-sec="investpro"   → #section-investpro
    │   └── 子頁籤 (inv-tab / switchInvestproTab)
    │       └── 年度損益   tab='yearlypnl'   → #investpro-sub-yearlypnl
    ├── 分析       data-sec="analysis"    → #section-analysis
    └── 操盤       data-sec="trading"     → #section-trading
```
